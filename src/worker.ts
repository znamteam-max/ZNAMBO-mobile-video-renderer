...
export class RenderContainer extends Container {
  ...
  static outboundByHost = {
    'r2.local': async (request: Request, env: Env) => {
      ...
      const key = safeKey(decodeURIComponent(url.pathname.slice(1)));
...
async function api(request: Request, env: Env) {
  ...
  if (request.method === 'POST' && path === '/api/upload/create') {
    ...
    const upload = await env.MEDIA.createMultipartUpload(safeKey(body.key), {
  ...
  if (request.method === 'POST' && path === '/api/upload/complete') {
    ...
    try {
      const obj = await env.MEDIA
        .resumeMultipartUpload(safeKey(body.key), body.uploadId)
        .complete(body.parts);
  ...
  if (request.method === 'POST' && path === '/api/render') {
    ...
    await env.RENDER_QUEUE.send({ jobId, config });
  ...
  if (request.method === 'GET' && jobMatch) {
    ...
    return job ? json(job) : bad('Job not found', 404);
  }
  ...
  async queue(batch: MessageBatch<RenderMessage>, env: Env) {
    for (const message of batch.messages) {
      ...
      try {
        ...
        const container = env.RENDERER.getByName(`job-${jobId}`);
        ...
        if (!response.ok) {
          throw new Error(payload.error || `Renderer HTTP ${response.status}`);
        }
